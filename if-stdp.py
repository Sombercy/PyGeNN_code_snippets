...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar  8 14:36:00 2020
...
stdp_params = {"tauPlus": 5.0, "tauMinus": 20.0, "aPlus": A_PLUS, "aMinus": A_MINUS, "wMin": 0.0, "wMax": G_MAX}
...
for i, (pre, post, w) in enumerate(zip(neuron_layers[:-1], neuron_layers[1:], weights)):
    if i==0:{
        ...
        model.add_synapse_population(
            "synapse%u" % i, "DENSE_INDIVIDUALG", NO_DELAY, pre, post,
            stdp_multiplicative, stdp_params, stdp_init, stdp_pre_init, stdp_post_init, "ExpCurr", post_syn_params, {})}
...
for a, s, l in zip(axes, layer_spikes, neuron_layers):
    ...
    a.set_xlim((0, DURATION_MS * TIMESTEP))
    a.set_ylim((0, l.size))
